...
const createNewPost = asyncHandler(async (req, res) => {
    ...
    const post = await Post.create({ user, title, text, date, location, prize, imagesrc, cloudinaryId })
...
const updatePost = asyncHandler(async (req, res) => {
    ...
    // Confirm data
    if (!id || !user || !title || !text || !date || !location || !prize || typeof completed !== 'boolean') {
        return res.status(400).json({ message: 'All fields are required' })
    }
