...
const createNewComment = asyncHandler(async (req, res) => {
    ...
    const duplicate = await Comment.findOne({ text }).collation({ locale: 'en', strength: 2 }).lean().exec()
    ...
    const comment = await Comment.create({ text, postId, createdById, createdByUsername })
